...
export default function GroupDetailScreen() {
  ...
  const { group, members, albums, loading, error, refresh } = useGroupDetail(
    groupId ?? null,
  );
    ...
    >
      ...
      <View className="mb-6 flex-row items-center">
        ...
        <Pressable
          ...
          onPress={() => router.push(`/home/${groupId}/members`)}
        >
          ...
          <View className="flex-1">
            ...
            <Text className="mt-1 text-sm text-textMuted">
              {group?.joinCode} · {group?.memberCount ?? 0} members
            </Text>
          </View>
        </Pressable>
        ...
        {/* INVITE */}
        <Pressable
          onPress={() => router.push(`/home/${groupId}/invite`)}
          ...
          ListEmptyComponent={() => (
            <View className="mt-10 items-center rounded-3xl bg-card p-8 shadow-lg">
              ...
              <Text className="mt-2 text-center text-textMuted">
                Create an album and start sharing memories.
              </Text>
            </View>
          )}
          renderItem={({ item }) => (
            <Pressable
              className="mb-4 w-[48%]"
              onPress={() =>
                router.push(`/home/${groupId}/albums/${item.albumId}`)
              }
            >
      ...
      <View className="absolute bottom-8 left-5 right-5 rounded-3xl p-3 shadow-lg">
        <Pressable
          onPress={() => router.push(`/home/${groupId}/create-album`)}
          className="items-center rounded-3xl bg-primary py-5"
        >
          <Text className="text-base font-bold text-white">Create Album</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
