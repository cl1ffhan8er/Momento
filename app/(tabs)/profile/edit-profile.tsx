...
export default function EditProfileScreen() {
  ...
  const { profile, loading, updateProfile, updateAvatarImage } = useProfile(
    user?.uid ?? null,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Unable to save profile", error.message ?? "Try again later");
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        <View className="flex-1 justify-center px-6 py-10">
          ...
          <View className="rounded-3xl bg-card p-6 shadow-lg">
            ...
            >
              <Text className="text-xs text-textMuted">Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
